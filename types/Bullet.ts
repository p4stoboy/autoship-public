...
export interface BulletTypeRegistry {
	create: (shipId: string, pos: Vec2, dir: number, speed: number, damage: number, battleState: BattleState) => BaseBullet;
	update: (bullet: BaseBullet, battleState: BattleState) => void;
	render: (ctx: CanvasRenderingContext2D, bullet: BaseBullet) => void;
	onCollision?: (bullet: BaseBullet, cell: BaseCell, shipId: string, battleState: BattleState) => void;
}
